...
## Converter defect fix: 2026-09-21

The earlier claim that no converter changes were needed was incorrect for public backbone-only checkpoints. `scripts/dinov2-to-gguf.py` now resolves the underlying DINOv2 module structurally through the model's `base_model` and module tree. This supports both direct `AutoModel` backbones and `AutoModelForImageClassification` wrappers without assuming the version-specific `model.dinov2` or `model.dinov2_with_registers` attributes. Encoder QKV extraction uses the resolved module's `encoder.layer`.

Classifier conversion remains selected only for the existing ImageNet checkpoint naming convention. Classifier labels and tensors continue to be emitted only from that classifier path; backbone-only conversion writes no invented head or labels and remains feature-mode compatible. `CONTRIBUTING.md` now names the supported public backbone-only model families. `tests/test_dinov2_converter.py` provides a lightweight synthetic direct-backbone, wrapped-classifier, and rejection smoke test.

Verification for this fix: `python -m py_compile scripts/dinov2-to-gguf.py tests/test_dinov2_converter.py` passed. `python -m unittest -v tests/test_dinov2_converter.py` ran two tests and skipped both because `torch` is not installed. No public weights were downloaded, so a real conversion was not run and no GGUF was created. C++ sources were unchanged; `cmake --build --preset debug -j2` reported no work and `ctest --test-dir build-debug --output-on-failure` passed all 3 tests. `git diff --check` passed. No merge, release, or push was performed.

## Converter hardening fix note: 2026-09-21

The resolver now treats objects that do not support `vars()` as leaves instead of allowing its `TypeError` to escape. Backbone shape matching also checks an available `config.model_type`, accepting DINOv2 (including register variants) or config-less synthetic objects while rejecting generic ViT/DeiT-shaped modules. Tests cover both malformed slot-only inputs and a non-DINO config. No dependency setup or public weights were added; the converter test remains skipped when `torch`, Transformers, or gguf are unavailable.
