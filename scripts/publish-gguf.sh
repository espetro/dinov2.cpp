...
# project editable plus the HuggingFace CLI, converts the model, and uploads
# the resulting ggml-model.gguf to dinov2-cpp-core/<variant>-gguf.
...
if [ ! -d "$VENV_DIR" ]; then
    ...
    echo "==> installing project (editable) + huggingface_hub[hf_transfer]"
    # -e . pulls in all [project] dependencies (torch, transformers, gguf, ...).
    # huggingface_hub is the only runtime dep not declared in pyproject.toml;
    # [hf_transfer] pulls in hf_transfer for HF_HUB_ENABLE_HF_TRANSFER=1.
    uv pip install -e . 'huggingface_hub[hf_transfer]'
else
