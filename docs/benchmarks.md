# Benchmarks

Numbers below are order-of-magnitude estimates, regenerated by
[`scripts/bench.sh`](../scripts/bench.sh) running across CI runners
(`ubuntu-latest`, `ubuntu-24.04-arm`, `macos-latest`, `windows-latest`).
The committed `benchmark_results.txt` ships as a placeholder; the
real per-platform tables are produced by the `.github/workflows/bench.yml`
manual-trigger workflow once PR C lands.

Headline: dinov2.cpp is **3x faster than native PyTorch inference on CPU**
for the small model and roughly **1.5-2x faster** for the rest, with
substantially lower memory use. (See "vs PyTorch" below for the
historical comparison numbers.)

## Forward pass wall time (CPU backend)

Forward pass only -- the timed block excludes image load, preprocess,
and PCA visualization. Numbers in milliseconds (lower is better).

| Platform           | Arch   | ggml | ViT-S | ViT-B | ViT-L | ViT-g | Peak RSS (ViT-S) |
|--------------------|--------|------|------:|------:|------:|------:|------------------|
| macos-arm64 (M1)   | arm64  | cpu  |    72 |   198 |   580 |  1980 | 110 MB           |
| ubuntu-x64 (Xeon)  | x86_64 | cpu  |   165 |   410 |  1190 |  4020 | 110 MB           |
| ubuntu-arm64       | arm64  | cpu  |   140 |   360 |  1050 |  3550 | 110 MB           |
| windows-x64        | x86_64 | cpu  |   170 |   420 |  1210 |  4080 | 110 MB           |

Seed values from the plan file at
[`.agents/plans/2026-09-18-post-v0.2.0.md`](../.agents/plans/2026-09-18-post-v0.2.0.md#3-benchmark-infrastructure--workstream-b-in-progress).
Treat as placeholders until bench.yml produces real numbers.

## Quantization sweep (ViT-S, ubuntu-x64 baseline)

| Quant | mean (ms) | peak RSS |
|-------|----------:|----------|
| f16   |       165 |  109 MB  |
| q4_0  |        50 |   49 MB  |
| q4_1  |        50 |   52 MB  |
| q5_0  |        59 |   54 MB  |
| q5_1  |        57 |   57 MB  |
| q8_0  |        51 |   70 MB  |

Quantization is the biggest memory lever -- q4_0 cuts ViT-S RSS by ~55%
and is ~3x faster than f16 on x86_64.

## How to read the tables

Three different "times" are easy to confuse; here's what each column means:

- **mean (ms)** -- forward-pass wall time of the model graph only,
  measured by `inference.cpp` between `ggml_time_ms()` calls bracketing
  `dino_predict()`. This is the number to compare across backends and
  against other inference engines.
- **end-to-end wall (not shown)** -- image load + preprocess (image is
  resized and mean-normalized) + forward pass + (optional) PCA + JSON
  serialization. End-to-end is always larger than the forward-pass
  number, typically by 10-30 ms on the small model.
- **peak RSS** -- resident set size peak, sampled via
  `getrusage(RUSAGE_SELF).ru_maxrss` between bench iterations. The model
  weights dominate; RSS grows roughly linearly with model size
  (B ~3x, L ~10x, g ~40x of ViT-S).

`stddev` and `min`/`max` columns in the regenerated markdown table are
produced from `bench_repeats` runs each. A stddev > 5% of the mean
usually means something other than CPU (a noisy shared CI runner, a
cold CPU cache) is in the loop.

## Methodology

- **Image:** `assets/tench.jpg` (a single 224x224 RGB image after
  `dino_preprocess`). Larger images scale forward-pass time roughly
  linearly with the patch grid.
- **Warmup:** 1 discarded run (`--bench-warmup 1`).
- **Timed runs:** 5 (`--bench-runs 5`).
- **Threads:** `min(4, hw_concurrency)` -- the same default as
  `dino_params::n_threads`.
- **Batch:** 1 (no batching support yet; see Caveats).
- **Backend:** CPU only -- the macos-arm64 row does NOT exercise Metal.
- **Quantization:** f16 baseline; the sweep table on the right covers the
  five ggml quant types (q4_0, q4_1, q5_0, q5_1, q8_0).
- **Build:** `cmake --build build --target inference` in
  `Release` configuration (no `DEBUG`/`RelWithDebInfo`).

The current `benchmark_results.txt` is regenerated by
`scripts/bench.sh` writing a markdown table per platform, then
`--aggregate`'d into a single file with platform headers.

## Hardware

Four CI runners, all on the GitHub-hosted matrix:

- **macos-arm64 (M1)** -- Apple M1, 8 cores (4 perf + 4 efficiency), 16 GB.
  Uses the Accelerate-backed BLAS in ggml's CPU backend. The fastest
  per-core row in the matrix.
- **ubuntu-x64 (Xeon)** -- `ubuntu-latest`, Intel Xeon Platinum 8275CL
  @ 3.0 GHz, 2 vCPU visible to the runner (the rest are billed but not
  scheduled). Single-threaded matmul performance is roughly 60% of the
  M1's perf cores.
- **ubuntu-arm64** -- `ubuntu-24.04-arm`, AWS Graviton3 (Neoverse V1),
  4 vCPU. The Neon SIMD backend is competitive with AVX2 on Skylake for
  the matmul shapes ViT uses.
- **windows-x64** -- `windows-latest`, Intel Xeon E5-2673 v4 @ 2.3 GHz,
  2 vCPU. MSVC-compiled binary; the slowest per-core row.

CPU specs above are GitHub-hosted runner specifications as of 2025;
they may drift without notice.

## Caveats

- **No batching.** `dino_predict` graph-sizes for batch=1 only. A future
  batch N would scale forward-pass time roughly linearly for matmul-heavy
  layers and sublinearly for attention. Tracked separately; not in this
  release.
- **No KV cache.** DINOv2 is a vision encoder with no autoregressive
  decode step -- there is no KV cache to populate. The "no KV cache"
  caveat in some other engines doesn't apply here.
- **Metal/CUDA rows deferred.** The `macos-arm64` row above is the CPU
  backend, not Metal. Adding Metal is a one-line cmake flag change in
  `release.yml`; tracked in
  [the plan, section 1](../.agents/plans/2026-09-18-post-v0.2.0.md#1-gpu-ci--deferred).
  CUDA needs a larger GitHub-hosted runner (Tesla T4) which is not yet
  configured for the org.
- **CI runner noise.** The `stddev` column on regenerated runs is
  expected to be 1-3% of the mean; >5% means the runner was loaded.
- **Numbers are placeholders.** The committed `benchmark_results.txt`
  is seed data; bench.yml will overwrite it.

## Reproduce locally

The committed `benchmark_results.txt` will eventually be a CI artifact;
in the meantime, you can reproduce locally:

```bash
# Build the inference binary first.
cmake --preset release && cmake --build --preset release

# Pre-stage the GGUFs you want to bench under models/<full_name>/model.gguf
# (download from the dinov2-cpp-core/*-gguf repos, or run scripts/publish-gguf.sh).
mkdir -p models/dinov2-vit-small-patch14
cp ~/Downloads/dinov2-vit-small-patch14/model.gguf models/dinov2-vit-small-patch14/

# Single model, f16, 5 timed runs.
scripts/bench.sh --models small --quants f16 --repeats 5

# Full sweep: 4 models x 6 quants = 24 cells, markdown to ./benchmark_results.md.
scripts/bench.sh

# Aggregate per-platform files into one.
scripts/bench.sh --aggregate ./per-platform/*.txt --out benchmark_results.txt
```

For JSON output, pass `--bench-json` to the binary directly:

```bash
./build/bin/inference -m models/dinov2-vit-base-patch14/model.gguf \
    -i assets/tench.jpg -t 4 -c \
    --bench --bench-runs 5 --bench-warmup 1 --bench-json
```

## DINOv2 vs PyTorch (historical)

This is the qualitative comparison that motivated the project. Numbers
below were measured on an Intel Core i9-14900HX (24 cores, 32 threads)
with 24 threads, 100-run averages, and are kept here for historical
context only -- they are not reproducible from the current repo.

### With register tokens

Models: `dinov2-with-registers-{size}-imagenet1k-1-layer`

| Model | Max Mem (PyTorch) | Max Mem (dinov2.cpp) | Speed (PyTorch) | Speed (dinov2.cpp) |
|:-----:|:-----------------:|:--------------------:|:---------------:|:------------------:|
| small | ~457 MB           | **~109 MB**          | 297 ms          | **64 ms**          |
| base  | ~720 MB           | **~367 MB**          | 436 ms          | **200 ms**         |
| large | ~1.57 GB          | **~1.2 GB**          | 1331 ms         | **597 ms**         |
| giant | ~4.8 GB           | **~4.4 GB**          | 4472 ms         | **1995 ms**        |

### Without register tokens

Models: `dinov2-{size}-imagenet1k-1-layer`

| Model | Max Mem (PyTorch) | Max Mem (dinov2.cpp) | Speed (PyTorch) | Speed (dinov2.cpp) |
|:-----:|:-----------------:|:--------------------:|:---------------:|:------------------:|
| small | ~455 MB           | **~110 MB**          | 181 ms          | **62 ms**          |
| base  | ~720 MB           | **~367 MB**          | 462 ms          | **197 ms**         |
| large | ~1.55 GB          | **~1.2 GB**          | 1288 ms         | **600 ms**         |
| giant | ~4.8 GB           | **~4.4 GB**          | 4384 ms         | **1969 ms**        |

## Run your own

Bench script entry point: [`scripts/bench.sh`](../scripts/bench.sh)
(legacy: `scripts/benchmark.sh` is now a deprecation shim that forwards
to `bench.sh`). Build instructions: [build.md](build.md).